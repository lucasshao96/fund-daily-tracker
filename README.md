...
- ⚠️ 最大回撤 + 风险评估
- 🎯 规则评分 + 定投建议（加仓/正常/减少/暂停）
- 🤖 AI 增强分析（Anspire 大模型，可选）
- 📧 HTML 精美邮件（纯文本自动降级）
- 🌍 全球市场行情（可选）
- 🏢 基金经理/规模/重仓股（可选）
...
> 💡 **如何找基金代码：** 支付宝/天天基金/银行 APP 搜基金名称，详情页都有 6 位数字代码。同一只基金的代码在所有平台都是一样的（证监会统一分配）。
...
| `ANSPIRE_API_KEYS` | [Anspire](https://open.anspire.cn/?share_code=0IDAOSJW) API Key（可选，用于 AI 增强分析。格式: `sk-xxxx`） | ❌ |
| `ANSPIRE_MODEL` | AI 模型选择（默认 `qwen3.5-flash`，最便宜。可选 `deepseek-v4-pro`、`qwen3.5-plus` 等） | ❌ |
...
每天北京时间 **14:30** 自动推送。要加减基金直接改 `FUND_LIST` 就行。
